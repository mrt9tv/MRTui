...
public class Program
{
    public static async Task Main(string[] args)
    {
        ...
        var subscriptionTask = client.SubscribeToAllStreams(
            ...
            onConnectStateChanged: async state => Console.WriteLine($"Connection: {state.State}"),
            onError: async error => Console.WriteLine($"Error: {error.Exception.Message}"),
...
For selective data consumption, use direct channel access:
...
|------------------|---------|-------------|
| `SubscribeToAllStreams` | All streams with optional delegates | Multiple |
| Direct channel access | High-frequency telemetry data (60Hz) | `TelemetryDataStream` |
| Direct channel access | Parsed session information | `SessionDataStream` |
| Direct channel access | Raw YAML session data | `RawSessionDataStream` |
| Direct channel access | Connection state changes | `ConnectStateStream` |
| Direct channel access | Error notifications | `ErrorStream` |
...
// NEW (Channel Extensions - recommended with async callbacks):
var subscriptionTask = client.SubscribeToAllStreams(
    onTelemetryUpdate: async data => { /* handle */ },
    onSessionInfoUpdate: async session => { /* handle */ },
    onError: async error => { /* handle */ },
...
| **Direct Channel Access** | Maximum performance, custom logic | Highest (650K+ records/sec) | Medium |
...
**Pattern:**
```csharp
// Single method handles all streams with async callbacks
var subscriptionTask = client.SubscribeToAllStreams(
    ...
    onConnectStateChanged: async state => { /* handle connection */ },
    onError: async error => { /* handle errors */ },
    cancellationToken: cancellationToken
);

await Task.WhenAny(client.Monitor(cancellationToken), subscriptionTask);
```

### Direct Channel Access Approach (Maximum Performance)
...
await Task.WhenAny(client.Monitor(cancellationToken), telemetryTask, sessionTask);
```
...
4. **Is this a prototype or simple application?**
   ...
   - No → Consider Direct Channel Access
...
client.ConnectStateStream          // ChannelReader<ConnectState> - connection changes
client.ErrorStream                 // ChannelReader<Exception> - error notifications
...
var clientOptions = new ClientOptions { MeterFactory = meterFactory };
await using var client = TelemetryClient<TelemetryData>.Create(logger, clientOptions, ibtOptions);
```
...
{
    await foreach (var state in client.ConnectStateStream.ReadAllAsync(cancellationToken))
    {
        switch (state)
        {
            case ConnectState.Connected:
...
// With metrics
var clientOptions = new ClientOptions { MeterFactory = meterFactory };
await using var client = TelemetryClient<TelemetryData>.Create(logger, clientOptions);
```
...
```csharp
var subscriptionTask = client.SubscribeToAllStreams(
    onTelemetryUpdate: async data => { /* handle data */ },
    onSessionInfoUpdate: async session => { /* handle session */ },
    onConnectStateChanged: async state => { /* handle connection */ },
    onError: async error => { /* handle errors */ },
    cancellationToken: cts.Token
);
...
await Task.WhenAny(client.Monitor(cts.Token), telemetryTask);
```
